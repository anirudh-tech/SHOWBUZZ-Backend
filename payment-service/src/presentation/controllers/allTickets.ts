...
export const allTickets = (dependencies: IDependencies) => {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const tickets: any = await Payment.find().skip(skip).limit(limit).populate('theatreId').populate('movieId').populate('userId')
